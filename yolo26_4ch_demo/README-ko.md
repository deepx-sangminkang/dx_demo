...
### `dxstream` 백엔드 설치 (vendored dx_stream)

기본 설치는 **legacy** 백엔드만 다룹니다. 네이티브
[`dxstream`](#추론-백엔드-engine_backend) 백엔드는 추가로 dx_stream GStreamer
플러그인과 `pydxs` Python 바인딩이 필요합니다. 이들은 `third_party/dxstream/`에
**in-tree로 vendoring**되어 있어 별도의 `dx_stream` 체크아웃이 필요 없습니다.
다음으로 빌드하세요:

```bash
./install.sh --with-dxstream          # 데모 + vendored dxstream 빌드 전체
# 또는 단독 실행 (idempotent):
./scripts/build_vendored_dxstream.sh  # (install_dxstream.sh가 이리로 forward)
```

`build_vendored_dxstream.sh`:
- 데모가 쓰는 element(`dxpreprocess` / `dxinfer` / `dxpostprocess` / `dxscale`)와
  `pydxs`가 읽는 metadata만 vendored 소스에서 빌드합니다 — dx_stream `git clone`
  불필요, `rdkafka`/`libmosquitto`/`eigen3` 의존성도 제거됩니다.
- 플러그인 + `pydxs`가 이미 사용 가능하면 빌드를 건너뜁니다(`--force`로 재빌드).
- 기본적으로 플러그인을 `third_party/dxstream/install`에 로컬 설치합니다(`sudo`
  불필요). `--prefix=PATH`로 변경 가능하며, `--skip-deps`로 빌드 도구 apt 설치를
  건너뜁니다.
- `scripts/.dxstream_env.sh`를 생성하며, `run_demo.sh`가 이를 자동으로 source해서
  `~/.bashrc` 수정 없이 플러그인을 인식합니다.

> vendored 플러그인은 여전히 **DX-RT**(`dx_engine`), GStreamer, OpenCV, json-glib,
> libyuv(RK3588에서는 `librga` 추가)에 링크됩니다. 이들은 시스템 사전 요구사항으로
> 남으며, 제거된 것은 별도의 dx_stream 패키징 단계뿐입니다.
