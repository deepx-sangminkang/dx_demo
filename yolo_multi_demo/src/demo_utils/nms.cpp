...
{
    int totalClasses = static_cast<int>(numClass) - startClass;
    std::vector<std::vector<BoundingBox>> perClassResults(totalClasses);

    #pragma omp parallel for schedule(dynamic)
    for(int cls = startClass; cls < static_cast<int>(numClass); cls++)
    {
        NmsOneClass(cls, ClassNames, ScoreIndices, Boxes, Keypoints,
                    IouThreshold, perClassResults[cls - startClass]);
    }

    for(auto &pcr : perClassResults)
    {
        Result.insert(Result.end(), pcr.begin(), pcr.end());
    }
