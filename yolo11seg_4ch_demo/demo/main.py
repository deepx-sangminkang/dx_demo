"""YOLOv11 멀티 채널 Qt 데모 스켈레톤.

- 채널별 캡처 스레드 + 전역 pre/infer/post 워커 + Qt GUI 2x2 레이아웃
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _update_performance_metrics(self, meta: Dict[str, Any]) -> None:
        ...
        # post 단계는 post(engine) + post(filter)의 합, draw 단계는 분리해서 측정한다.
    ...
    예: "ch1 fps=29.8 proc=1234 drop(input=10 infer=5 post=3 draw=1)"
        """
    ...
    def _init_engine_and_workers(self, config: Dict[str, Any]) -> None:
        ...
        # wait → postprocess 로 전달되는 결과 큐
        self.post_queue: "queue.Queue[Any]" = queue.Queue(maxsize=32)
        # postprocess → draw 로 전달되는 결과 큐
        ...
        # postprocess 워커 풀 생성 (post_queue → draw_queue)
    ...
    def closeEvent(self, event: QtGui.QCloseEvent) -> None:  # pragma: no cover
        ...
        if hasattr(self, '_threads'):
            for _ in range(len(self._threads)):
                for q in [self.input_queue, self.infer_queue, self.post_queue, self.draw_queue]:
                    try:
                        q.put_nowait(None)
                    except queue.Full:
                        pass
