...
### `dxstream` 백엔드 설치 (dx_stream)

기본 설치는 **legacy** 백엔드만 다룹니다. 네이티브
[`dxstream`](#추론-백엔드-engine_backend) 백엔드는 추가로 dx_stream GStreamer
플러그인과 `pydxs` Python 바인딩이 필요합니다. 다음으로 설치하세요:

```bash
./install.sh --with-dxstream          # 데모 + dx_stream 전체
# 또는 단독 실행 (idempotent):
./scripts/install_dxstream.sh
```

`install_dxstream.sh`:
- dx_stream + `pydxs`가 이미 설치돼 있으면 빌드를 건너뜁니다(`--force`로 재빌드).
- dx_stream 소스를 `$DX_STREAM_SRC`, 일반적인 형제 경로에서 찾거나,
  `$DX_STREAM_REPO`(기본 `git@github.com:DEEPX-AI/dx_stream`)에서 클론합니다.
- dx_stream의 `install.sh`(시스템 의존성)와 `build.sh`(플러그인 + `pydxs`)를
  실행합니다. `--dxstream-skip-deps` / `--skip-deps`로 시스템 의존성 단계를
  건너뛸 수 있습니다.
- `scripts/.dxstream_env.sh`를 생성하며, `run_demo.sh`가 이를 자동으로 source해서
  `~/.bashrc` 수정 없이 플러그인을 인식합니다.

> dx_stream은 **DX-RT**(`dx_engine`)에 링크되므로, 빌드 전에 위의 동일한
> 사전 요구사항이 충족돼야 합니다.
